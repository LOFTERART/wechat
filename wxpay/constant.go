...
const (
	...
	BankTypeJzcbDebit     = "JZCB_DEBIT"     // 锦州银行(借记卡)
	BankTypeKlbDebit      = "KLB_DEBIT"      // 昆仑银行(借记卡)
	BankTypeKrcbDebit     = "KRCB_DEBIT"     // 昆山农商(借记卡)
	BankTypeKuerlecbDebit = "KUERLECB_DEBIT" // 库尔勒市商业银行(借记卡)
	BankTypeLjbDebit      = "LJB_DEBIT"      // 龙江银行(借记卡)
	BankTypeNyccbDebit    = "NYCCB_DEBIT"    // 南阳村镇银行(借记卡)
	BankTypeLsccbDebit    = "LSCCB_DEBIT"    // 乐山市商业银行(借记卡)
	BankTypeLuzbDebit     = "LUZB_DEBIT"     // 柳州银行(借记卡)
	BankTypeLwbDebit      = "LWB_DEBIT"      // 莱商银行(借记卡)
	BankTypeLyyhbDebit    = "LYYHB_DEBIT"    // 辽阳银行(借记卡)
	BankTypeLzbDebit      = "LZB_DEBIT"      // 兰州银行(借记卡)
	BankTypeMintaibCredit = "MINTAIB_CREDIT" // 民泰银行(信用卡)
	BankTypeMintaibDebit  = "MINTAIB_DEBIT"  // 民泰银行(借记卡)
	BankTypeNcbDebit      = "NCB_DEBIT"      // 宁波通商银行(借记卡)
	BankTypeNmgnxDebit    = "NMGNX_DEBIT"    // 内蒙古农信(借记卡)
	BankTypeXabDebit      = "XAB_DEBIT"      // 西安银行(借记卡)
	BankTypeWfbCredit     = "WFB_CREDIT"     // 潍坊银行(信用卡)
	BankTypeWfbDebit      = "WFB_DEBIT"      // 潍坊银行(借记卡)
